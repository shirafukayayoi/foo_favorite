...
#define uCallNextHookEx CallNextHookEx
	...
	enum
	{
		uPrintCrashInfo_max_length = 1024
	};
	...
	static inline void uPrintf(pfc::string_base &out, const char *fmt, ...)
	{
		va_list list;
		va_start(list, fmt);
...
{
	pfc::string8 temp;
	if (!uGetDlgItemText(wnd, id, temp))
		temp = "";
	return temp;
}
...
inline void uStringLower(pfc::string_base &out, const char *src, t_size len = ~0)
{
	out.reset();
	uAddStringLower(out, src, len);
}
inline void uStringUpper(pfc::string_base &out, const char *src, t_size len = ~0)
{
	out.reset();
	uAddStringUpper(out, src, len);
}
...
/* relevant types not yet defined here */ template <typename t_stream, typename t_abort>
void t_font_description::to_stream(t_stream p_stream, t_abort &p_abort) const
{
...
/* relevant types not yet defined here */ template <typename t_stream, typename t_abort>
void t_font_description::from_stream(t_stream p_stream, t_abort &p_abort)
{
